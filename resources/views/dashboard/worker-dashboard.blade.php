    ...
    <style>
        ...
        *------------------*/
        ...
        /* ***************
            my-account End
        ******************/
        .btn-warning { background-color: #ce5cf6 !important; border-color: #ce5cf6 !important; color: white !important; }
        .btn-check:focus+.btn-warning, .btn-warning:focus { box-shadow: 0 0 0 0.25rem rgb(206 92 246 / 95%) !important; }
       .f_c{ color: #1f1f1f !important; }
        .form-control { background-color: #ffffff !important; height: 35px !important; border: 1px solid #da5bfe7a !important; }
        .nav { display: flex; padding-left: 0; margin-bottom: 0; list-style: none; flex-direction: column !important; }
                                                ...
                                                <td>
                                                    ...
                                                    <div class="d-flex">
                                                        <form onsubmit="changeStatus(event)">
                                                            ...
                                                            <button type="submit" style="padding: 0 3px;" class="btn btn-xs btn-warning shadow-none">
                                                                <i class='bx bx-loader-circle' style="font-size: 17px"></i>
                                                            </button>
                                                        </form>
                                                        &nbsp;
                                                        <a href="{{ url('view-worker-order-details',$item->id) }}" title="View Order Details" style="padding: 0 3px;background-color: #e2bcff;border: #e2bcff; border-radius: 3px;" class="shadow-none">
                                                            <i class='bx bx-show' style="font-size: 17px;padding-top: 3px;"></i>
                                                        </a>
                                                    </div>
                                                    @elseif($item->status == 'proccess')
                                                        <button onclick="showModal(event, {{$item->id}}, {{$item->due}})" type="button" style="padding: 0 3px;background-color: #e2bcff;border: #e2bcff; border-radius: 3px;" class="shadow-none">
                                                            <i class="bx bx-check-square" style="font-size: 17px;padding-top: 3px;"></i>
                                                        </button>
                                                    @else
                                                    @endif
                                                </td>
                        ...
                        <form onsubmit="changeStatus(event)">
                            ...
                            <div class="modal-body">
                                <div class="row">
                                    ...
                                    {{-- <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="billAmount">Due Amount</label>
                                            <input type="number" min="0" step="0.01" class="form-control" id="billAmount" oninput="calculateTotal(event)" name="billAmount" value="0" />
                                        </div>
                                    </div> --}}
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="number" min="0" step="0.01" disabled class="form-control" id="paidAmount" oninput="calculateTotal(event)" name="paidAmount" value="0" />
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
...
@push("front_script")
    <script>
        ...
        function showModal(event, id, due) {
            $("#staticBackdrop").modal('show');
            $("#staticBackdrop").find('#id').val(id);
            $("#staticBackdrop").find('#dueAmount').val(due);
        ...
        function calculateTotal(event) {
            ...
            let dueAmount = $("#staticBackdrop").find('#dueAmount').val();

            let paidAmount = (billAmount - dueAmount);
            // let dueAmount = (billAmount - paidAmount);
            // $("#staticBackdrop").find('#dueAmount').val(parseFloat(dueAmount).toFixed(2));
            $("#staticBackdrop").find('#paidAmount').val(parseFloat(paidAmount).toFixed(2));
        }
